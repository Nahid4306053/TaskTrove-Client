...
export default function AddTask({CloseModel}) {
  ...
  return (
    <div>
     ...
     <div className="flex"> <h1 className="p-5 mx-auto px-10 text-xl text-center bg-base-300   clipshape2 shadow-lg">
         Add New Task
        </h1>
        </div>
      <form onSubmit={handelinfo} className="mt-10">
        <div className="form-control">
          ...
          <input type="text" placeholder="You Task Name" className="input focus:outline-none input-bordered" name="name" required />
        </div>
        ...
        <div className="form-control mt-2">
          ...
          <textarea rows={5} name="description" required className="textarea textarea-bordered focus:outline-none placeholder:text-base" placeholder="Type What You will do in this Task" ></textarea>
   
        </div>   
        <div className=" mt-4  flex items-center gap-5">  
        <DatePicker selected={startDate} onChange={(date) => setStartDate(date)} customInput={<div className="btn btn-sm bg-base-300 overflow-hidden relative ">Deu date </div>} />
        <div className="badge-secondary badge ">{moment(startDate).calendar().includes('at') ? moment(startDate).calendar().split(" at ")[0] : moment(startDate).calendar()}</div>
        </div>
