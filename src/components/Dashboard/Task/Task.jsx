    ...
    <div className='bg-red-200  p-5 border-b' ref={drag} style={{ opacity: isDragging ? 0.9 : 1 }} >
      <div className='flex  justify-between items-center'>
        <div className="handel">
          <h2  onClick={()=>(settaskID(task._id),document.getElementById('viewTask').showModal())} className='md:text-lg hover:text-red-400 cursor-pointer'><i className="fa-regular mr-5 fa-list-check"></i>{task.name}</h2>
          ...
          <div>
          <div onClick={()=>(settaskID(task._id),document.getElementById('viewTask').showModal())} className="btn btn-xs   capitalize">view  task</div>
          </div>

          <div>
          <div onClick={()=>(settaskID(task._id),document.getElementById('Edit').showModal())} className="btn btn-xs capitalize">Edit task</div>
          </div>          
          <div>
