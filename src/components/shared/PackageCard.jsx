      ...
      <div className="card-body mt-5">
        <h2 className="text-xl">
          <Link
            ...
            to={`/singel-package/${data._id}`}
          >
            {data.title}
          </Link>
        </h2>
        <p>
          <span className="text-lg text-sky-500 ">${data.price}</span>
          <span className="text-sm ml-1 opacity-60">/ Per person</span>
        </p>
        <div className="card-actions gap-5 items-center mt-5">
          <div className="day flex items-center gap-2">
            <GoClock /> {data.day} Days
          </div>
        ...
        <div className="card-actions mt-2">
          ...
          <div className="badge badge-outline ">
            {data.destination.place.split("-")[0]}
          </div>
        </div>
      </div>
    </div>
